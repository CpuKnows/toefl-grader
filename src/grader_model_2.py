...
def process_essays(load_testing_files=False):
	...
	if load_testing_files==True:
		...
		for filename in testing_essay_key['filename']:
			with open('../input/testing/essays/'+filename, 'r') as f:
	...
	for essay in essay_key['essay']:
		print("...", essay_key.loc[essay_i, 'filename'])
		essay_i = essay_i + 1
...
#	index=False)
testing_df[['filename','a','b','c','cii','ciii','di','dii','final_score','final_grade']].to_csv(
